...
이 폴더는 GitHub에서 프로젝트 구성을 이해할 수 있도록 넣어둔 소량 샘플만 포함한다.
...
- 전체 학습 데이터와 전처리 결과는 `wake_word/data/` 아래에 있고, `.gitignore`로 제외된다.
- 모델 산출물과 탐색 결과도 `wake_word/models/` 아래에서 관리되며, 공개 리포에는 포함하지 않는다.
