...
- 이 문서는 원래 wake word positive 데이터 생성 관점에서 정리한 조사다.
- 따라서 여기서 말하는 "목소리 다양성"은 `"하이 포포"` 계열 짧은 학습 샘플 생성 목적에 맞춰진 기준이다.
- 현재 제품 TTS 방향성은 이 문서만으로 결정하지 않는다.
- 대화형 한국어 TTS 선택, zero-shot 검토, custom training 보류 판단은 `tts/README.md`와 `docs/research/tts.md`를 기준으로 본다.
