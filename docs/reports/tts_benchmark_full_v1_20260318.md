...
- 사람 listening score 입력 전에도 자동 평가 기준선과 Jetson 이식 우선순위를 정할 수 있게 한다.
...
| 모델 | family | CER | exact | RTF | peak VRAM MB | 메모 |
|---|---|---:|---:|---:|---:|---|
| `OpenAI API TTS (KO)` | reference | 0.0496 | 0.50 | 0.2799 | 0 | 품질 상한 reference, 네트워크 의존 |
| `Edge TTS (KO)` | reference | 0.0803 | 0.39 | 0.1811 | 0 | wake word 생성 baseline, 네트워크 의존 |
| `MeloTTS (KO)` | local | 0.0999 | 0.42 | 0.1462 | 1522 | 현재 한국어 로컬 후보 중 가장 균형적 |
| `OpenVoice V2 (KO)` | local | 0.1130 | 0.33 | 0.1970 | 1886 | cloning 가능성은 있으나 현재 자동 점수는 불리 |

자동 평가만 놓고 보면 한국어 로컬 기본 후보는 여전히 `MeloTTS`다.

## 영어 엔진 표

| 모델 | family | CER | exact | RTF | peak VRAM MB | 메모 |
|---|---|---:|---:|---:|---:|---|
| `OpenAI API TTS (EN)` | reference | 0.0483 | 0.67 | 0.2696 | 0 | 품질 상한 reference |
| `Kokoro (EN)` | local | 0.0541 | 0.68 | 0.0483 | 1400 | 현재 영어 로컬 1위, 속도도 가장 좋음 |
| `Piper (EN)` | local | 0.0590 | 0.66 | 0.0645 | 950 | 경량성 강점, 영어 runtime 후보 가치 높음 |
| `Edge TTS (EN)` | reference | 0.0622 | 0.64 | 0.4051 | 0 | 듣기 baseline용 reference |
| `MeloTTS (EN)` | local | 0.0647 | 0.60 | 0.0712 | 1480 | 영어도 가능하지만 현재 점수는 Kokoro/Piper보다 낮음 |
| `OpenVoice V2 (EN)` | local | 0.0776 | 0.53 | 0.1516 | 1872 | reference를 바꿔도 영어 자동 점수는 중하위권 |

영어 로컬 후보는 현재 `Kokoro`와 `Piper`가 우선 검토 대상이다.
