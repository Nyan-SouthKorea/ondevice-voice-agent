...
현재 1차 집중 범위는 wake word와 VAD 요소기술 완료 이후, STT를 고정 문장 50개 기준 데이터셋으로 비교 평가하는 단계까지 확장됐다.
...
- STT는 직접 녹음한 고정 문장 50개 세트로 속도와 정확도를 비교한다.
...
지금은 `Wake Word`와 `VAD` 요소기술을 각각 완료했고, STT는 먼저 고정 문장 50개 녹음 데이터셋으로 여러 모델을 비교한 뒤 기본값을 정하는 전략이다.
...
샘플은 third-party dataset 원본이 아니라 직접 생성한 TTS만 유지하는 보수적 기준을 사용한다.
예시 샘플 경로는 `wake_word/examples/audio_samples/`를 사용한다.
