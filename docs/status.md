...
| STT | 비교 평가 완료 후 기본값 확정 | 온디바이스 기본 후보는 `WhisperTRT small nano safe`, 비교 기준은 `Whisper base (PyTorch + CUDA)` |
...
5. `WhisperTRT small nano safe`를 기준으로 wake word + VAD + STT 통합 파이프라인을 연결한다.
6. Jetson에서 `MeloTTS` 설치와 한국어 합성 smoke를 검증한다.
7. STT GUI 데모의 2모델 비교 방식을 기준으로 통합 GUI 데모를 구현한다.
