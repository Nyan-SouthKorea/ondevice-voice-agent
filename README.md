...
현재는 `하이 포포` wake word와 `VAD` 요소기술 개발을 마쳤고, 다음 단계로 두 모듈을 연결한 뒤 `STT`, `LLM`, `TTS`까지 결합해 Jetson Orin Nano에서 실시간으로 동작하는 전체 음성 에이전트 스택으로 확장하는 것이 목표다.
...
- 이 리포에는 코드와 문서 중심으로 남기고, 대용량 데이터와 민감 운영 정보는 분리 관리한다.
...
현재 구현이 끝난 요소기술은 [`wake_word`](wake_word/README.md)와 [`vad`](vad/README.md)다. [`stt`](stt/README.md)는 초기 래퍼 구현과 비교 평가 준비까지 완료했다.

Wake word:
- negative 데이터셋 준비 완료
  - `AI Hub + MUSAN + FSD50K`
- positive clean / mixed augmentation 완료
- feature extraction, baseline 학습, grid search, full-data 최종 학습 완료
- ONNX export 완료
- Jetson 실시간 추론 및 GUI demo 완료
...
VAD:
- `VADDetector` 공통 진입점 구현 완료
- `webrtcvad` / `Silero ONNX` dual backend 구현 완료
- 기본 backend를 `silero`로 확정
- 최소 filtering(`min_speech_frames=3`, `min_silence_frames=10`) 적용 완료
- 기본 마이크 기준 terminal demo 검증 완료

현재 남은 작업:
- 실제 마이크 기준 wake word threshold와 input gain 확정
- hard negative / 연속 배경 오디오 기준 false accept 점검
- wake word와 VAD를 연결해 STT 입력 구간 절단 기준 정리
- STT 50문장 직접 녹음 데이터셋으로 `tiny / base / small`과 API 경로 비교
- 상위 SDK 연결용 인터페이스 정리
...
- 다만 재현성과 Jetson 데모 실행에 직접 필요한 소형 runtime ONNX 자산은 예외적으로 포함할 수 있다.
- 외부 데이터셋 원본 샘플은 이 리포에 포함하지 않는다.
- 예시 오디오 샘플은 `wake_word/examples/audio_samples/` 아래의 직접 생성한 소량 샘플만 유지한다.
- 내부 주소, 계정 식별자, SSH 경로 같은 민감한 운영 정보는 문서에 직접 남기지 않고 `secrets/` 아래의 로컬 전용 문서에서 관리한다.

## 문서 허브

세부 문서는 [docs/README.md](docs/README.md)에서 한 번에 찾을 수 있다.  
현재 구현 상태를 가장 빠르게 확인하려면 [docs/status.md](docs/status.md), [wake_word/README.md](wake_word/README.md), [vad/README.md](vad/README.md)를 먼저 보면 된다.
