...
Jetson Orin Nano 기준의 온디바이스 로봇 음성 에이전트를 단계적으로 구축하는 리포지토리다. 현재는 wake word와 VAD 요소기술을 마무리했고, STT 기본 온디바이스 모델을 `WhisperTRT small nano safe`로 확정했다. 다음 우선순위는 TTS를 Jetson 기준으로 붙여 재사용 가능한 음성 에이전트 SDK 형태로 확장하는 것이다.

## 현재 범위

- wake word: 학습, 평가, ONNX export, Jetson 실시간 demo 완료
- VAD: 공통 detector와 기본 backend 정리 완료
- STT: 공통 래퍼, 직접 녹음 recorder, 비교 평가 완료, 기본 온디바이스 모델 `WhisperTRT small nano safe` 확정
- TTS: 공통 래퍼와 API 최소 합성 경로 준비, 다음 우선 개발 대상
...
| [stt](stt/README.md) | 음성 인식 | 기본 온디바이스 모델 확정, 통합 데모 유지 |
| [tts](tts/README.md) | 음성 합성 | 초기 구조 구현, 다음 우선 개발 대상 |
...
로컬 워크스페이스는 보통 `repo/`, `env/`, `secrets/`를 같은 루트 아래 sibling으로 둔다. `secrets/`는 리포 바깥 `../secrets/`에서 관리하고, 재생성 가능한 로컬 실행 결과는 필요할 때만 `../results/`를 만들어 사용한다. 리포의 공식 현재 상태와 운영 기준은 `docs/` 아래 문서를 기준으로 유지한다.
